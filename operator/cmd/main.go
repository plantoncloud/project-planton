...
func init() {
	...
	utilruntime.Must(plantonaiv1.AddToScheme(scheme))
...
func main() {
	...
	// own election and double-reconcile every platform. Refuse to start with
	// a self-explanatory error instead; the check re-runs each restart, so
	// removing the other installation heals this one without intervention.
	// Outside a cluster (make run) there is no namespace and nothing to guard.
	if ownNamespace, inCluster := singleton.OwnNamespace(); inCluster {
		guardClient, err := client.New(restConfig, client.Options{Scheme: scheme})
		if err != nil {
			setupLog.Error(err, "Failed to create client for the one-operator-per-cluster check")
			os.Exit(1)
		}
	...
	mgr, err := ctrl.NewManager(restConfig, ctrl.Options{
		...
		LeaderElectionID:       "42bd8b33.planton.ai",
