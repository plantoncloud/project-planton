...
- **PerconaServerMongoDBRestore** (optional) — when `restore` is declared, a run named after the declaration that replays the named backup (and, with PITR, the archived oplog) into the cluster once its members are up
...
- For keyless S3 backups: cloud-side ambient AWS identity on the member pods (EKS IRSA or node instance-profile credentials). GCS backups always need a service-account key — a `GcpServiceAccount` with a user-managed key, granted `roles/storage.objectAdmin` on the bucket, supplies it by reference.
...
**Backups are a deliberate choice, not a default** — the omitted backup block means NO backups. Declare named storage destinations (S3/S3-compatible, Cloudflare R2, GCS, or Azure Blob per destination), schedule tasks against them by name with FIVE-field cron expressions (`"0 2 * * *"` is daily at 02:00), and enable PITR to archive the oplog continuously so a restore can land BETWEEN backups. With multiple storages, exactly one carries `main: true` — PITR chunks and restore metadata land there. One `prefix` per cluster (PBM keeps its metadata under it); the source and its restore target deliberately share one, two live clusters never do. PITR starts archiving only after the first backup succeeds, and its recovery point is the last closed chunk (`oplogSpanMin`). Logical backups restore anywhere; physical backups are much faster at scale; incrementals build on an `incremental-base`.

**Credential posture per backend** — S3 on real AWS supports the keyless posture (omit `accessKeys`; the pods' IRSA or instance-profile identity signs). GCS does NOT: Percona Backup for MongoDB authenticates to Google Cloud Storage with a service-account key only, so every GCS storage declares `credentials` — the key itself (raw, or a `GcpServiceAccount`'s `key_base64` output wired by reference) or the name of a Secret already carrying it. Grant that account `roles/storage.objectAdmin` AND `roles/storage.legacyBucketReader` on the bucket, and rotate the key like a password (recreate the `GcpServiceAccount` key and re-apply). Cloudflare R2 has its own arm (`r2`), declared in R2's terms — the bucket, its account and jurisdiction, and a Cloudflare API token, each by reference to the catalog's `CloudflareR2Bucket` and `CloudflareAccountApiToken` — and the module composes the jurisdiction's endpoint, region `auto`, path-style addressing, and the token as an S3 key pair; R2 has no keyless posture from any cluster. A token scoped to the single bucket with `Workers R2 Storage Bucket Item Write` is enough for logical backups, PITR chunks, and a point-in-time restore (live-proven). Cloudflare refuses to delete a bucket that still holds objects and the provider has no force-destroy, so a retired backup bucket is emptied over the S3 API before its `CloudflareR2Bucket` is destroyed. Other S3-COMPATIBLE endpoints (MinIO, Ceph RGW, GCS over its S3-interoperability endpoint with HMAC keys) always need declared keys — nothing ambient can mint their credentials. Every declared credential is stored as a managed-secret reference, never plaintext.

**Restore is declared on the cluster that receives the data** — `restore` names a backup (a Backup object in this namespace, or the backup's location inside one of the cluster's own declared storages when another cluster took it) and optionally a point in time (`latest`, or a `date`). The Restore object is created only after the cluster reports ready (the operator's own "agents are up" check passes while a fresh cluster's backup agents are still restarting through the replica set's first authentication, and a Restore created that early is parked in a terminal "get backup meta: not found"); the operator then syncs the store's metadata and replays the backup into the running cluster — the module copies the storage definition into the restore object because the operator resolves a backup-source restore's storage from the restore itself during that sync. A restore that fails stays failed (`kubectl get psmdb-restore` shows the error; fix the cause and change the declaration to run again). Two rules make a cross-cluster restore work: the receiving cluster declares the SAME storage the backup lives in, and it points `systemUsersSecretName` at the source cluster's `{source}-secrets` — a restored database carries the source's users and passwords, and the operator must log in with them. Back that Secret up with the data.
...
**GKE replica set with GCS backups and disaster recovery** — three members spread over three nodes, nightly logical backups plus PITR into a GCS bucket with the key wired by reference from a `GcpServiceAccount`, and a documented restore target that declares the same storage, a `restore` block, and the source's system-users Secret. Start from the **GKE Replica Set with GCS Backups** preset; the full resource set is in the guide.
